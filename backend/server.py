...
async def admin_create_content(body: ContentCreate, current_admin: AdminUser = Depends(get_current_admin)):
    slug = body.slug or await unique_slug_for_title(body.title)
    content = Content(**{**body.dict(), 'slug': slug})
...
async def admin_update_content(content_id: str, body: ContentUpdate, current_admin: AdminUser = Depends(get_current_admin)):
    ...
    if 'title' in update_doc and 'slug' not in update_doc:
        update_doc['slug'] = await unique_slug_for_title(update_doc['title'])
...
async def on_startup():
    ...
    if await db.content.count_documents({}) == 0:
        samples = [
            {
                'id': str(uuid.uuid4()), 'title': 'Squid Game', 'original_title': '오징어 게임', 'slug': 'squid-game',
            ...
            {
                'id': str(uuid.uuid4()), 'title': 'Your Name', 'original_title': '君の名は。', 'slug': 'your-name',
