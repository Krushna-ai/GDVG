...
const Navbar: React.FC<NavbarProps> = ({ 
  ...
  const navLinkClass = (view: 'home' | 'mylist' | 'series' | 'films' | 'people_catalog') => 
    `text-sm md:text-base font-medium transition cursor-pointer ${
        currentView === view ? 'text-white font-bold' : 'text-gray-300 hover:text-white'
    }`;

  const mobileLinkClass = (view: 'home' | 'mylist' | 'series' | 'films' | 'people_catalog') => 
    `text-xl font-medium transition cursor-pointer block py-2 ${
        currentView === view ? 'text-red-600 font-bold' : 'text-gray-300 hover:text-white'
    }`;
      ...
      <div className="container mx-auto px-4 md:px-12 flex justify-between items-center">
        <div className="flex items-center space-x-4 md:space-x-8">
          ...
          <nav className="hidden md:flex items-center space-x-6">
            ...
            <button onClick={() => onNavigate('series')} className={navLinkClass('series')}>Series</button>
            <button onClick={() => onNavigate('films')} className={navLinkClass('films')}>Films</button>
            <button onClick={() => onNavigate('people_catalog')} className={navLinkClass('people_catalog')}>Celebs</button>
            <button onClick={() => onNavigate('mylist')} className={navLinkClass('mylist')}>My List</button>
          </nav>
        </div>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 bg-black/95 flex flex-col animate-fadeIn md:hidden">
           ...
           <nav className="flex-grow flex flex-col p-8 space-y-4">
                <button onClick={() => { onNavigate('home'); setIsMobileMenuOpen(false); }} className={mobileLinkClass('home')}>Home</button>
                <button onClick={() => { onNavigate('series'); setIsMobileMenuOpen(false); }} className={mobileLinkClass('series')}>Series</button>
                <button onClick={() => { onNavigate('films'); setIsMobileMenuOpen(false); }} className={mobileLinkClass('films')}>Films</button>
                <button onClick={() => { onNavigate('people_catalog'); setIsMobileMenuOpen(false); }} className={mobileLinkClass('people_catalog')}>Celebs</button>
                <button onClick={() => { onNavigate('mylist'); setIsMobileMenuOpen(false); }} className={mobileLinkClass('mylist')}>My List</button>
