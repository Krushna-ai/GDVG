...
export async function generateStaticParams() {
  return generateContentStaticParams(['drama'], 1000);
}

export async function generateMetadata({ params }: { params: Promise<{ id: string; slug: string }> }): Promise<Metadata> {
  const resolvedParams = await params;
  return generateContentMetadata(resolvedParams, 'drama');
}

export default async function DramaDetailPage({ params }: { params: Promise<{ id: string; slug: string }> }) {
  const resolvedParams = await params;
