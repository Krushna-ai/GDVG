...
export async function generateStaticParams() {
  return generateContentStaticParams(['movie'], 1000);
}

export async function generateMetadata({ params }: { params: Promise<{ id: string; slug: string }> }): Promise<Metadata> {
  const resolvedParams = await params;
  return generateContentMetadata(resolvedParams, 'movies');
}

export default async function MovieDetailPage({ params }: { params: Promise<{ id: string; slug: string }> }) {
  const resolvedParams = await params;
